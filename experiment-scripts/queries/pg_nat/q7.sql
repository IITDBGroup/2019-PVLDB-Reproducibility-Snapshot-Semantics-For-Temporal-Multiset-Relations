...
SELECT * FROM
        ...
        (select * from time_lineitem where l_shipdate > TO_DATE('1995-01-01', 'YYYY-MM-DD') AND l_shipdate < TO_DATE('1996-12-31', 'YYYY-MM-DD')) b
        ON (s_suppkey = l_suppkey)) x1
),
temp_view2 AS (
SELECT * FROM 
(temp_view1 
...
PERIOD JOIN WITH (ts4, te4, ACTIVE_TIME_BEGIN, ACTIVE_TIME_END) AS (ts, te)
                      ...
                      WHERE ((n_name = 'EGYPT' AND n_name1 = 'IRAQ') OR (n_name = 'IRAQ' AND n_name1 = 'EGYPT'))
),
temp_view6 AS (
...
_temp_view_2 AS (
SELECT supp_nation AS "supp_nation", cust_nation AS "cust_nation", sum(volume) AS "revenue", ts AS "t_b", te AS "t_e" FROM temp_view6
